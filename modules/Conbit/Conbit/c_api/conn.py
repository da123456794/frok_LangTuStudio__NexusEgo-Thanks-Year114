...
if platform.uname()[0] == "Windows":
    lib_path = f"Conbit_windows_{machine}.dll"
    lib_path = os.path.join(os.path.dirname(__file__),"libs", lib_path)
    LIB = ctypes.cdll.LoadLibrary(lib_path)
elif platform.uname()[0] == "Linux":
    lib_path = f"Conbit_linux_{machine}.so"
    lib_path = os.path.join(os.path.dirname(__file__),"libs", lib_path)
    print(lib_path)
    LIB = ctypes.CDLL(lib_path)
else:
    lib_path = f"Conbit_macos_{machine}.dylib"
    lib_path = os.path.join(os.path.dirname(__file__),"libs", lib_path)
    LIB = ctypes.CDLL(lib_path)
...
class ConnectType(enum.Enum):
    Remote="Remote" # 连接到一个 Conbit Access Point
    Local="Local" # 在内部启动一个单独的 Conbit Core
...
if __name__ == '__main__':
    ...
    if conn_type==ConnectType.Local:
        # 直接在内部启动一个 Conbit, 不需要 fb,omega 也不需要新进程
        # 你可以把它当成一个普通函数
        # 因为是在内部启动的，所以需要账号密码
        ...
        # 远程连接到一个已经启动的 Conbit Access Point
