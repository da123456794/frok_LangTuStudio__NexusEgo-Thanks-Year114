...
func invalidAuthServerURLError() error {
	return fmt.Errorf("invalid auth server URL, expected like http://127.0.0.1:1200")
}

func normalizeLooseAuthServerURL(authURL string) string {
	switch {
	case strings.HasPrefix(authURL, "http//:"):
		return "http://" + strings.TrimPrefix(authURL, "http//:")
	case strings.HasPrefix(authURL, "https//:"):
		return "https://" + strings.TrimPrefix(authURL, "https//:")
	case strings.HasPrefix(authURL, "http:/") && !strings.HasPrefix(authURL, "http://"):
		return "http://" + strings.TrimPrefix(authURL, "http:/")
	case strings.HasPrefix(authURL, "https:/") && !strings.HasPrefix(authURL, "https://"):
		return "https://" + strings.TrimPrefix(authURL, "https:/")
	default:
		return authURL
	}
...
func jsonDecodeTypedResp[T any](resp *http.Response) (T, error) {
	var ret T
	if resp.StatusCode == 503 {
		return ret, errors.New(i18n.T(i18n.S_auth_server_is_down_503))
	}
	_body, _ := io.ReadAll(resp.Body)
	body := string(_body)
	if resp.StatusCode != 200 {
		return ret, parseError(body)
	}
	if err := json.Unmarshal([]byte(body), &ret); err != nil {
		return ret, fmt.Errorf(i18n.T(i18n.S_error_parsing_auth_server_api_response), err)
	}
	return ret, nil
}

func NormalizeAuthServerURL(authUrl string) (string, error) {
	authUrl = strings.TrimSpace(authUrl)
	authUrl = normalizeLooseAuthServerURL(authUrl)
	authUrl = regexp.MustCompile(`/+$`).ReplaceAllString(authUrl, "")
	parsedURL, err := url.Parse(authUrl)
	if err != nil {
		return "", invalidAuthServerURLError()
	}
...
func CreateClient(options *ClientOptions) (*Client, error) {
	normalizedAuthServer, err := NormalizeAuthServerURL(options.AuthServer)
	if err != nil {
		return nil, err
	}
	options.AuthServer = normalizedAuthServer
...
func (client *Client) TanLobbyAuth(roomID string, fbtoken string) (tanauth.TanLobbyLoginResponse, error) {
	roomID = strings.TrimSpace(strings.ReplaceAll(roomID, "：", ":"))
	roomID = strings.TrimPrefix(roomID, "TanLobby:")
	request := tanauth.TanLobbyLoginRequest{
		FBToken: fbtoken,
		RoomID:  roomID,
	}
	...
	if err != nil {
		return tanauth.TanLobbyLoginResponse{}, fmt.Errorf("TanLobbyAuth: %v", err)
	}
	tanLobbyLoginResp, err := jsonDecodeTypedResp[tanauth.TanLobbyLoginResponse](resp)
	if err != nil {
		return tanauth.TanLobbyLoginResponse{}, fmt.Errorf("TanLobbyAuth: %v", err)
	}
...
func (client *Client) TanLobbyCreate(fbtoken string) (tanauth.TanLobbyCreateResponse, error) {
	request := tanauth.TanLobbyCreateRequest{
		FBToken: fbtoken,
	}
	requestJSONBytes, _ := json.Marshal(request)
	resp, err := client.client.Post(
	...
	if err != nil {
		return tanauth.TanLobbyCreateResponse{}, fmt.Errorf("TanLobbyCreate: %v", err)
	}
	tanLobbyCreateResp, err := jsonDecodeTypedResp[tanauth.TanLobbyCreateResponse](resp)
	if err != nil {
		return tanauth.TanLobbyCreateResponse{}, fmt.Errorf("TanLobbyCreate: %v", err)
	}
...
func (client *Client) TanLobbyDebug(loginResponse string, raknetRand []byte, fbtoken string) (tanauth.TanLobbyDebugResponse, error) {
	request := tanauth.TanLobbyDebugRequest{
		FBToken:       fbtoken,
	...
	if err != nil {
		return tanauth.TanLobbyDebugResponse{}, fmt.Errorf("TanLobbyDebug: %v", err)
	}
	tanLobbyDebugResp, err := jsonDecodeTypedResp[tanauth.TanLobbyDebugResponse](resp)
	if err != nil {
		return tanauth.TanLobbyDebugResponse{}, fmt.Errorf("TanLobbyDebug: %v", err)
	}
