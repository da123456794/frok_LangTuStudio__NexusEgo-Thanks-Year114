...
import (
	...
	"github.com/LangTuStudio/Conbit/utils/sync_wrapper"
	"github.com/google/uuid"
)

type AreaRequester struct {
	ctrl               Conbit.GameIntractable
	react              Conbit.PacketDispatcher
	entityUniqueID     int64
	extendInfo         Conbit.ExtendInfo
	structuresMu       sync.Mutex
	structureListeners map[string][]func(Conbit.StructureResponse)
	subChunkListeners  *sync_wrapper.HybridListener[Conbit.SubChunkResult]
}
...
func (a *AreaRequester) onStructureResponse(pk *packet.StructureTemplateDataResponse) {
	...
	if !ok && len(a.structureListeners) == 1 {
		for name, candidate := range a.structureListeners {
			listeners = candidate
			delete(a.structureListeners, name)
			ok = true
			break
		}
	}
...
func (o *AreaRequester) LowLevelRequestStructure(pos define.CubePos, size define.CubePos, structureName string) async_wrapper.AsyncResult[Conbit.StructureResponse] {
	return async_wrapper.NewAsyncWrapper(func(ac *async_wrapper.AsyncController[Conbit.StructureResponse]) {
		o.structuresMu.Lock()
		_, ok := o.structureListeners[structureName]
		if !ok {
			o.structureListeners[structureName] = make([]func(Conbit.StructureResponse), 0, 1)
		}
		o.structureListeners[structureName] = append(o.structureListeners[structureName], func(sr Conbit.StructureResponse) {
			ac.SetResult(sr)
		})
...
func (o *AreaRequester) LowLevelRequestStructureWithAutoName(pos define.CubePos, size define.CubePos) async_wrapper.AsyncResult[Conbit.StructureResponse] {
...
func (c *AreaRequester) SetOnSubChunkResult(nonBlockingCallback func(Conbit.SubChunkResult)) {
	c.subChunkListeners.SetNonBlockingFixListener(nonBlockingCallback)
}
func (c *AreaRequester) AttachSubChunkResultListener(nonBlockingCallback func(Conbit.SubChunkResult)) (detachFn func()) {
	return c.subChunkListeners.AttachNonBlockingDetachableListener(nonBlockingCallback)
}

func NewAreaRequester(ctrl Conbit.GameIntractable, react Conbit.PacketDispatcher, uq Conbit.MicroUQHolder, info Conbit.ExtendInfo) *AreaRequester {
	d := &AreaRequester{
		ctrl:               ctrl,
		react:              react,
		...
		extendInfo:         info,
		structuresMu:       sync.Mutex{},
		structureListeners: make(map[string][]func(Conbit.StructureResponse)),
		subChunkListeners:  sync_wrapper.NewHybridListener[Conbit.SubChunkResult](),
	}
