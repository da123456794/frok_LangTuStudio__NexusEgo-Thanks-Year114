...
import (
	...
	"github.com/LangTuStudio/Conbit/utils/structure/pos_operations"
...
type BotActionHighLevel struct {
	...
	cmdHelper        Conbit.CommandHelper
	areaRequester    Conbit.LowLevelAreaRequester
	microAction      Conbit.BotAction
	pickedItemChan   chan protocol.InventoryAction
	playerHotBarChan chan *packet.PlayerHotBar
	muChan           chan struct{}
	node             defines.Node
	// asyncNBTBlockPlacer Conbit.AsyncNBTBlockPlacer
	nameCount int
}
...
func (o *BotActionHighLevel) highLevelRequestLargeArea(startPos define.CubePos, size define.CubePos, dst chunks.ChunkProvider, withMove bool) error {
	...
	for _, xRange := range chunkRangesX {
		...
		for _, zRange := range chunkRangesZ {
			...
			var err error
			for i := 0; i < 3; i++ {
				var resp Conbit.StructureResponse
				var structure Conbit.DecodedStructure
				if err != nil {
