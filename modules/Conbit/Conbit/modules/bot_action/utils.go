...
func RefreshPosAndDimensionInfo(e *Conbit.PosAndDimensionInfo, omega Conbit.CmdSender, botRuntimeID uint64, preferAIQueryTarget bool) error {
	var resp *packet.CommandOutput
	var err error
	if preferAIQueryTarget {
		resp, err = omega.SendAICommandNeedResponse(fmt.Sprintf("%d", botRuntimeID), "querytarget @s").SetTimeout(3 * time.Second).BlockGetResult()
	} else {
		resp, err = omega.SendWebSocketCmdNeedResponse("querytarget @s").SetTimeout(3 * time.Second).BlockGetResult()
	}
	if resp == nil || err != nil {
		return fmt.Errorf("cannot get bot dimension info")
	} else {
		var QueryResults []Conbit.QueryResult
