...
func NewAccessPointInteractCore(node defines.APINode, conn minecraft_conn.Conn) Conbit.InteractCore {
...
func NewAccessPointReactCore(node defines.Node, conn minecraft_conn.Conn) Conbit.UnStartedReactCore {
	core := NewReactCore()
	...
	pressureMetric := pressure_metric.NewPressureMetric(time.Second*3, func(e float32) {
		if e > 0.5 {
			termlog.Warnf("server->access pressure: %.2f%%", e*100)
		}
	...
	commandRespFreq := pressure_metric.NewFreqMetric(time.Second*5, func(e float32) {
		if e > 30 {
			termlog.Warnf(i18n.T(i18n.S_bot_is_sending_cmd_at_a_very_high_ratio_could_cause_stability_issue), e)
		}
	})
