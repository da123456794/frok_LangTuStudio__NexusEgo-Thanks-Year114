...
func init() {
	if false {
		func(Conbit.ReactCore) {}(&ReactCore{})
	}
}
...
type ReactCore struct {
	...
	noBlockAndDetachableCallbacks []*sync_wrapper.SyncKVMap[string, Conbit.NoBlockAndDetachablePacketCallback]
	deferredStart                 func()
...
func NewReactCore() *ReactCore {
	core := &ReactCore{
		...
		noBlockAndDetachableCallbacks: make([]*sync_wrapper.SyncKVMap[string, Conbit.NoBlockAndDetachablePacketCallback], 0, 400),
	...
	for i := 0; i < maxID; i++ {
		core.noBlockAndDetachableCallbacks = append(core.noBlockAndDetachableCallbacks, sync_wrapper.NewSyncKVMap[string, Conbit.NoBlockAndDetachablePacketCallback]())
...
func (r *ReactCore) AddNewNoBlockAndDetachablePacketCallBack(wants map[uint32]bool, cb Conbit.NoBlockAndDetachablePacketCallback) {
	for id := range wants {
...
func (r *ReactCore) handleNoBlockAndDetachablePacket(pk packet.Packet) {
	...
	subPumper.Iter(func(k string, pumper Conbit.NoBlockAndDetachablePacketCallback) (continueIter bool) {
		err := pumper(pk)
