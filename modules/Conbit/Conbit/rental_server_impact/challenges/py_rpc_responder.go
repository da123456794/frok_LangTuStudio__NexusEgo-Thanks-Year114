...
func NewPyRPCResponder(omega Conbit.MicroOmega, canSolveChallenge CanSolveChallenge) *PyRPCResponder {
...
func (o *PyRPCResponder) onPyRPC(pk packet.Packet) {
	...
	if command == "S2CHeartBeat" {
		...
		if len(data) < 1 {
			o.solveFail <- fmt.Errorf("%s", i18n.T(i18n.S_fail_to_get_start_type_data))
		}
		...
		if len(data) < 2 {
			o.solveFail <- fmt.Errorf("%s", i18n.T(i18n.S_fail_to_get_check_num_data))
		}
