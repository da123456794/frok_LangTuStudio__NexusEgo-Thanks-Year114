...
func init() {
	if false {
		func(Conbit.ExtendInfo) {}(&ExtendInfoHolder{})
	}
}
...
type ExtendInfoHolder struct {
	...
	GameRules           map[string]*Conbit.GameRule
	knownGameRules      bool
	Dimension           int32
	knownDimension      bool
...
func NewExtendInfoHolder(conn minecraft_conn.Conn) *ExtendInfoHolder {
	return &ExtendInfoHolder{
		GameRules:          make(map[string]*Conbit.GameRule),
...
func (e *ExtendInfoHolder) GetGameRules() (gameRules map[string]*Conbit.GameRule, found bool) {
...
func (e *ExtendInfoHolder) setGameRules(gameRuleName string, rule *Conbit.GameRule) {
...
func (uq *ExtendInfoHolder) UpdateFromPacket(pk packet.Packet) {
	...
	case *packet.GameRulesChanged:
		for _, r := range p.GameRules {
			uq.setGameRules(r.Name, &Conbit.GameRule{
