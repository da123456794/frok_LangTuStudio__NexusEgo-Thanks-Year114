...
## Conbit
### 目的
`Conbit` 提供了与 `Minecraft` 相关的工具、概念抽象和 `API` ，但是 仅仅包括 `Minecraft` 相关的 `API` 。
考虑到使用该库的项目可能包含了一些复杂的，很可能出现错误的业务逻辑，因此 `Conbit` 支持以远程附加形式工作，即:
...
- 机器人基本 `API` 及 `Minecraft` 相关基本 `API` 的封装
  ...
  - 所有支持的接口描述均位于 Conbit/*.go
- 方块转换和描述: Conbit/blocks
- c 接口 Conbit/c_api
- 区块转换和描述 Conbit/mirror
- 一些基本的例子可以在 Conbit/entries 下找到
...
### 相关库
- gophertunnel: 
https://github.com/LangTuStudio/Conbit  
...
- PhoenixBuilder  
https://github.com/LangTuStudio/Conbit  
PhoenixBuilder主要用于网易版MC的建筑导入导出，几何体构建等。Conbit 前身由 PhoenixBuilder 分叉出来
...
## afterword
### 声明
`Conbit-core` 原存储库已闭源，该存储库基于原存储库较新的开源版本

### 许可证声明
`PhoenixBuilder` 该存储库也已闭源，由于该存储库的最后一个开源版本使用 APGLv3 许可证
根据 AGPLv3 的条款，任何包含其代码的衍生作品在分发时，必须整体以 AGPLv3 授权。

因此，**本存储库（`Conbit-core` 的分支）的全部代码同样采用 AGPLv3 许可证发布**
