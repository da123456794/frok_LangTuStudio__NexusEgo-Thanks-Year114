...
const (
	...
	S_Conbit_access_point_starting                                                              = "access point starting..."
	S_Conbit_access_point_ready                                                                 = "access point ready"
	...
	S_omega_builder_auth_fail                                                                   = "builder auth fail: %v"
	S_omega_builder_api_auth_fail                                                               = "builder API auth fail: %v"
	...
	S_omega_launcher_config_file_and_programme_logic_mismatch_or_programme_logic_error_in_stage = "launcher config mismatch or programme logic error. stage: %v"
	S_executing_login_sequence                                                                  = "executing login sequence..."
	S_updating_tag_in_omega_net                                                                 = "updating local network properties"
...
var I18nDict_zh_cn map[string]string = map[string]string{
	...
	S_sending_request_network_settings_packet:                             "正在向 Minecraft %s请求网络参数",
	S_sending_login_packet:                                                "正在向 Minecraft %s请求游戏登录",
	S_sending_client_to_server_handshake_packet:                           "正在向 Minecraft %s确认握手请求",
	...
	S_connecting_to_mc_server:                                             "正在连接到网易%s(%s%v 密码: %v)...",
	...
	S_fail_connecting_to_mc_server_retrying:                               "无法连接到网易%s, 正在第 %v 次重新尝试连接到网易%s -> 请查看之前的输出，可能指明了无法连接的实际原因\n",
	S_done_connecting_to_mc_server:                                        "成功连接到网易%s",
	...
	S_checking_bot_op_permission_and_game_cheat_mode:                      "正在检查机器人的权限和%s内作弊模式是否打开...",
	S_done_checking_bot_op_permission_and_game_cheat_mode:                 "机器人的权限和%s内作弊模式检查完成，无问题",
	...
	S_unknown_code:                                                        "未知的验证服务器响应:%v -> 请截屏并反馈给开发者适配",
	S_reading_info:                                                        "读取登陆配置中...",
	S_Conbit_access_point_starting:                                        "[接入点]: 启动中...",
	S_Conbit_access_point_ready:                                           "[接入点]: 就绪",
	S_fail_to_dispatch_packet_to_remote:                                   "[接入点] -> [访问点] 数据包分发失败",
	...
	S_please_grant_bot_operator_privilege:                                 "缺少许可，请给予 %s 许可 (操作员权限)",
	...
	S_debug_remove_everything_under:                                       "Debug: 正在移除目录下所有接入点文件",
	S_connecting_to_access_point:                                          "[访问点]: 正在连接到 [接入点]",
	S_connected_to_access_point:                                           "[访问点]: 成功连接到 [接入点]",
	S_connecting_to_access_point_timeout_rebooting:                        "[访问点]: 与 [接入点] 的连接超时, 正在重连",
	S_autonomous_operator_booting:                                         "自律型 Operator 启动中...",
	S_autonomous_operator_waiting_for_access_point_connection:             "自律型 Operator 正在等待 [接入点] 建立与 Minecraft 服务器的连接",
	S_bot_is_sending_cmd_at_a_very_high_ratio_could_cause_stability_issue: "机器人正在以极高的速度发送命令 (%.2f 命令每秒) 这可能会造成稳定性问题",
	S_enhance_build_is_not_supported:                                      "您使用的程序构建并不包含导入模块 -> 这通常是因为下载来源不完整，请使用完整构建版本",
	S_omega_builder_auth_fail:                                             "导入器授权失败: %v -> 您未获得导入器使用权限，请联系销售人员",
	S_omega_builder_api_auth_fail:                                         "导入器 API 授权失败: %v -> 您未获得导入器 API 使用权限，请联系销售人员",
	S_programme_logic_error:                                               "程序设计存在缺陷 -> 请截屏并将错误信息反馈给开发者等待修复",
	...
	S_net_position_conflict_only_one_access_point_can_exist:               "网络位置冲突, 网络中已经存在 [接入点] -> 此问题很可能是程序逻辑错误，请截屏联系开发者",
	S_send_commandfeedback_is_set_to_be:                                   "根据启动时信息，将沿用设置: gamerule sendcommandfeedback=%v",
	S_detecting_send_command_feedback_mode:                                "正在检测租赁服内 sendcommandfeedback 模式",
	S_no_access_point_in_network:                                          "[接入点] 不存在于网络中或未启动 -> 请先启动 [接入点]",
	...
	S_omega_launcher_config_file_and_programme_logic_mismatch_or_programme_logic_error_in_stage: "启动器逻辑存在缺陷或配置文件版本与启动器不符(%v) -> 程序尝试自动排除错误，请重启，若错误反复出现请联系开发者反馈问题",
}
...
var I18nCodeDict_zh_cn map[int]string = map[int]string{
	...
	C_Auth_InvalidFBVersion:               "验证服务器: 你使用的程序与验证服务器协议版本不匹配 -> 请更新此程序，若更新后此问题仍然出现，请联系开发者适配协议",
