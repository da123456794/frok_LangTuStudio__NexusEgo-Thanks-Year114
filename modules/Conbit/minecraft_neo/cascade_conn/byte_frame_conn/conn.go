...
func NewConnectionFromNet(netConn interface {
	...
	return newConnectionFromNet(netConn, true)
}

func NewConnectionFromNetWithoutHeader(netConn interface {
	net.Conn
	WaitClosed() chan struct{}
}) *ByteFrameConnection {
	return newConnectionFromNet(netConn, false)
}

func newConnectionFromNet(netConn interface {
	net.Conn
	WaitClosed() chan struct{}
}, writeHeader bool) *ByteFrameConnection {
	enc := packet.NewEncoder(netConn)
	dec := packet.NewDecoder(netConn)
	if !writeHeader {
		enc = packet.NewEncoderWithoutHeader(netConn)
		dec = packet.NewDecoderWithoutHeader(netConn)
	}
