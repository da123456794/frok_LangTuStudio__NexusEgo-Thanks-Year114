...
func loginInfoLine(msg string) {
	termlog.Infof("%s", msg)
}

func (core *Core) serverKindName() string {
	if core.ServerKindName == "" {
		return "服务器"
	}
	return core.ServerKindName
}
...
type Core struct {
	*options.Options
	...
	packMu sync.Mutex
	// pendingPackets holds packets that arrived before they were expected in the login sequence.
...
func NewLoginAndSpawnCore(packetConn defines.PacketConn, opt *options.Options) *Core {
	core := &Core{
		...
		pendingPackets:     make([]packet.Packet, 0, 4),
	}
	go func() {
...
func (core *Core) Login(ctx context.Context) (err error) {
	...
	loginInfoLine(fmt.Sprintf(i18n.T(i18n.S_sending_request_network_settings_packet), core.serverKindName()))
	...
	case <-core.readyToLoginSignal:
		...
		loginInfoLine(fmt.Sprintf(i18n.T(i18n.S_sending_login_packet), core.serverKindName()))
