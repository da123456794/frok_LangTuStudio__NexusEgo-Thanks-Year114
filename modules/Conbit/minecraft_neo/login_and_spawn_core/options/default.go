...
import (
	...
	"github.com/LangTuStudio/Conbit/minecraft_neo/login_and_spawn_core/skin"
	"github.com/google/uuid"
)

const defaultGrowthLevel = 50
...
func NewTanLobbyOptions(
	...
	opt := &Options{
		...
		IdentityData: login.IdentityData{
			...
			DisplayName: userPlayerName,
			Uid:         int64(userUniqueID),
			GameType:    "LobbyGame",
		},
	}
	_, _ = rand.Read(opt.Salt)
	opt.ClientData = defaultClientData(address, authResp)
	opt.Request = login.EncodeOffline(opt.IdentityData, opt.ClientData, privateKey)
	identityData, _, _, err := login.Parse(opt.Request)
	if err != nil {
		panic(err)
	}
...
func defaultClientData(address string, authResp map[string]any) login.ClientData {
	...
	d.GrowthLevel = defaultGrowthLevel
