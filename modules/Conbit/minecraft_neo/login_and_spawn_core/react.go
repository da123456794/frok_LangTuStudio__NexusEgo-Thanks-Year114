...
import (
	...
	"github.com/LangTuStudio/Conbit/minecraft/resource"
...
func (core *Core) Receive(pk packet.Packet) {
	...
	if !readyToLoginBefore && core.readyToLogin {
		...
		default:
			termlog.Infof("resend ready signal")
		}
	}
	if !loggedInBefore && core.loggedIn {
		...
		default:
			termlog.Infof("resend logged signal")
		}
	}
}
...
func (core *Core) receive(pk packet.Packet) {
	if pk.ID() == packet.IDDisconnect {
		core.CloseWithError(fmt.Errorf("%s", pk.(*packet.Disconnect).Message))
		return
	}
	if core.loggedIn && !core.waitingForSpawn.Load() {
		return
	}
	if core.tryHandleExpectedPacket(pk) {
		core.drainPendingPackets()
		return
	}
	core.deferPacket(pk)
}

func (core *Core) tryHandleExpectedPacket(pk packet.Packet) bool {
	for _, id := range core.expectedIDs.Load().([]uint32) {
		if id == pk.ID() {
			if err := core.handlePacket(pk); err != nil {
				panic(err)
			}
			return true
		}
...
func (core *Core) deferPacket(pk packet.Packet) {
	core.pendingMu.Lock()
	defer core.pendingMu.Unlock()
	core.pendingPackets = append(core.pendingPackets, pk)
}

func (core *Core) drainPendingPackets() {
	for {
		core.pendingMu.Lock()
		if len(core.pendingPackets) == 0 {
			core.pendingMu.Unlock()
			return
		}
		remaining := core.pendingPackets[:0]
		progressed := false
		for _, pk := range core.pendingPackets {
			if core.loggedIn && !core.waitingForSpawn.Load() {
...
func (core *Core) handlePacket(pk packet.Packet) error {
	...
	case *packet.ItemRegistry:
		return core.handleItemRegistry(pk)
...
func (core *Core) handleClientToServerHandshake() error {
	...
	for _, pack := range core.resourcePacks {
		texturePack := protocol.TexturePackInfo{Version: pack.Version(), Size: uint64(pack.Len())}
		texturePack.UUID = uuid.MustParse(pack.UUID())
		if pack.Encrypted() {
...
func (core *Core) handleServerToClientHandshake(pk *packet.ServerToClientHandshake) error {
	...
	loginInfoLine(fmt.Sprintf(i18n.T(i18n.S_sending_client_to_server_handshake_packet), core.serverKindName()))
...
func (core *Core) handleResourcePacksInfo(pk *packet.ResourcePacksInfo) error {
	...
	for _, pack := range pk.TexturePacks {
		packUUID := pack.UUID.String()
		if _, ok := core.packQueue.downloadingPacks[packUUID]; ok {
			core.ErrorLog.Printf("duplicate texture pack entry %v in resource pack info\n", packUUID)
			core.packQueue.packAmount--
			continue
		}
		core.packQueue.downloadingPacks[packUUID] = downloadingPack{
			size:       pack.Size,
			buf:        bytes.NewBuffer(make([]byte, 0, pack.Size)),
			newFrag:    make(chan []byte),
			contentKey: pack.ContentKey,
		}
		core.ignoreResourcePack(packUUID, pack.Version)
	}
...
func (core *Core) handleResourcePackStack(pk *packet.ResourcePackStack) error {
	...
	for _, pack := range pk.TexturePacks {
		...
		if !core.hasPack(pack.UUID, pack.Version, false) {
			core.ignoreResourcePack(pack.UUID, pack.Version)
		}
	}
...
func (core *Core) ignoreResourcePack(uuid, version string) {
	core.packMu.Lock()
	...
	core.ignoredResourcePacks = append(core.ignoredResourcePacks, exemptedResourcePack{uuid: uuid, version: version})
}
...
func (core *Core) startGame() {
	...
	if len(data.Items) > 0 {
		_ = core.packetConn.WritePacket(&packet.ItemRegistry{Items: data.Items})
	}
	_ = core.packetConn.Flush()
...
func (core *Core) handleStartGame(pk *packet.StartGame) error {
	...
	core.expect(packet.IDItemRegistry)
	return nil
}

func (core *Core) handleItemRegistry(pk *packet.ItemRegistry) error {
	core.gameData.Items = pk.Items
	for _, item := range pk.Items {
		if item.Name == "minecraft:shield" {
			core.packetConn.SetShieldID(int32(item.RuntimeID))
		}
	}
	_ = core.packetConn.WritePacket(&packet.RequestChunkRadius{ChunkRadius: 16, MaxChunkRadius: 16})
...
func (core *Core) handleRequestChunkRadius(pk *packet.RequestChunkRadius) error {
	...
	if core.biomes == nil {
		...
		_ = nbt.UnmarshalEncoding(b, &biomes, nbt.NetworkLittleEndian)
		_ = core.packetConn.WritePacket(&packet.BiomeDefinitionList{
			SerialisedBiomeDefinitions: biomes,
		})
	} else {
		_ = core.packetConn.WritePacket(&packet.BiomeDefinitionList{SerialisedBiomeDefinitions: core.biomes})
	}
