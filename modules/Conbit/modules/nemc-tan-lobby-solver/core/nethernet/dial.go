...
func (d Dialer) DialContext(ctx context.Context, networkID uint64, signaling Signaling) (*Conn, error) {
	...
	tanDebugf("nethernet: start dial connectionID=%d localNetworkID=%d remoteNetworkID=%d", d.ConnectionID, signaling.NetworkID(), networkID)

	tanDebugf("nethernet: waiting credentials")
	credentials, err := signaling.Credentials(ctx)
	if err != nil {
		return nil, fmt.Errorf("obtain credentials: %w", err)
	}
	tanDebugf("nethernet: credentials ready")
	...
	gatherer.OnLocalCandidate(func(candidate *webrtc.ICECandidate) {
		if candidate == nil {
			tanDebugf("nethernet: local candidate gather finished count=%d", len(candidates))
			close(gatherFinished)
			return
		}
		candidates = append(candidates, *candidate)
		tanDebugf("nethernet: local candidate #%d %s", len(candidates), candidateSummary(*candidate))
	})
	tanDebugf("nethernet: gathering local candidates")
	...
	case <-gatherFinished:
		...
		tanDebugf("nethernet: offer sent")
		...
		tanDebugf("nethernet: local candidates sent count=%d", len(candidates))

		n, stop := d.notifySignals(networkID, signaling)
		tanDebugf("nethernet: waiting answer")
		select {
		case <-ctx.Done():
		...
		case signal := <-n.signals:
			...
			tanDebugf("nethernet: answer received bytes=%d", len(signal.Data))
			...
			tanDebugf("nethernet: waiting first remote candidate")
			select {
			case <-ctx.Done():
				if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			...
			case <-c.candidateReceived:
				...
				tanDebugf("nethernet: first remote candidate received")
...
func (d Dialer) startTransports(ctx context.Context, conn *Conn, desc *description) error {
	...
	tanDebugf("nethernet: starting ICE")
	...
	if pair, _ := conn.ice.GetSelectedCandidatePair(); pair != nil {
		tanDebugf("nethernet: ICE connected local=%s remote=%s", candidateSummary(*pair.Local), candidateSummary(*pair.Remote))
	} else {
		tanDebugf("nethernet: ICE connected without selected candidate pair")
	}

	conn.log.Debug("starting DTLS transport as client")
	tanDebugf("nethernet: starting DTLS")
	if err := withContext(ctx, func() error {
		return conn.dtls.Start(desc.dtls)
	}); err != nil {
		return fmt.Errorf("start DTLS: %w", err)
	}
	tanDebugf("nethernet: DTLS connected state=%s", conn.dtls.State().String())

	conn.log.Debug("starting SCTP transport")
	tanDebugf("nethernet: starting SCTP")
	if err := withContext(ctx, func() error {
		return conn.sctp.Start(desc.sctp)
	}); err != nil {
		return fmt.Errorf("start SCTP: %w", err)
	}
	tanDebugf("nethernet: SCTP connected")
	if err := withContext(ctx, func() error {
		var err error
	...
	tanDebugf("nethernet: ReliableDataChannel created")
	if err := withContext(ctx, func() error {
		var (
			err            error
	...
	tanDebugf("nethernet: UnreliableDataChannel created")
	return nil
}
	...
	for {
		...
		case signal := <-signals:
			...
			case SignalTypeCandidate, SignalTypeError:
				tanDebugf("nethernet: signal received type=%s bytes=%d", signal.Type, len(signal.Data))
...
func (d *dialerNotifier) NotifySignal(signal *Signal) {
	...
	tanDebugf("nethernet: notifier accepted signal type=%s bytes=%d", signal.Type, len(signal.Data))
	d.signals <- signal
}
