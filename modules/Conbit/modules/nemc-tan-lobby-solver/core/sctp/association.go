...
func (a *Association) sendInit() error {
	...
	sctpDebugf("sctp: queue INIT state=%s", getAssociationStateString(a.getState()))
...
func (a *Association) sendCookieEcho() error {
	...
	sctpDebugf("sctp: queue COOKIE-ECHO state=%s", getAssociationStateString(a.getState()))
...
func (a *Association) readLoop() {
	...
	for {
		...
		sctpDebugf("sctp: recv raw bytes=%d", n)
...
func (a *Association) marshalPacket(p *packet) ([]byte, error) {
	doChecksum := !a.sendZeroChecksum || chunkMandatoryChecksum(p.chunks)
	raw, err := p.marshal(doChecksum)
	if err != nil {
		return nil, err
	}
	sctpDebugf(
		"sctp: send state=%s chunks=%s bytes=%d checksum=%t zeroSend=%t",
		getAssociationStateString(a.getState()),
		chunkNames(p.chunks),
		len(raw),
		doChecksum,
		a.sendZeroChecksum,
...
func (a *Association) handleInbound(raw []byte) error {
	...
	if err != nil {
		...
		sctpDebugf("sctp: recv parse error=%v", err)
	...
	if err := checkPacket(pkt); err != nil {
		...
		sctpDebugf("sctp: recv validation error=%v chunks=%s", err, chunkNames(pkt.chunks))

		return nil
	}
	sctpDebugf(
		"sctp: recv state=%s chunks=%s bytes=%d checksumRequired=%t",
		getAssociationStateString(a.getState()),
		chunkNames(pkt.chunks),
		len(raw),
		!a.recvZeroChecksum,
	)
...
func (a *Association) handleForwardTSN(chunkTSN *chunkForwardTSN) []*packet {
	...
	// unordered chunks.
	// See https://github.com/LangTuStudio/Conbit
	for _, s := range a.streams {
...
func (a *Association) completeHandshake(handshakeErr error) bool {
	if handshakeErr != nil {
		sctpDebugf("sctp: handshake complete err=%v state=%s", handshakeErr, getAssociationStateString(a.getState()))
	} else {
		sctpDebugf("sctp: handshake complete state=%s", getAssociationStateString(a.getState()))
	}
	select {
