...
import (
	...
	"github.com/LangTuStudio/Conbit/utils/pressure_metric"

	// "github.com/LangTuStudio/Conbit/minecraft_neo/defines"
	"net"
	"sync"
	"time"
)
...
// packetCompressionAdapter adapts packet.Compression to underlay_conn.Compression
type packetCompressionAdapter struct {
	pkCompression packet.Compression
}

func (a *packetCompressionAdapter) Compress(decompressed []byte) ([]byte, error) {
	return a.pkCompression.Compress(decompressed)
}

func (a *packetCompressionAdapter) Decompress(compressed []byte) ([]byte, error) {
	return a.pkCompression.Decompress(compressed)
}

// packetCompressionAdapterFunc creates an adapter from packet.Compression to underlay_conn.Compression
func packetCompressionAdapterFunc(pkCompression packet.Compression) Compression {
	if pkCompression == nil {
		return nil
	}
	return &packetCompressionAdapter{pkCompression: pkCompression}
}
...
func (c *ByteFrameConnection) writeRoutine(period time.Duration) {
	...
	pressureMetric := pressure_metric.NewPressureMetric(time.Second*3, func(e float32) {
		if e > 0.5 {
			termlog.Warnf("net send pressure: %.2f%%", e*100)
		}
...
func (conn *ByteFrameConnection) EnableCompression(algorithm packet.Compression) {
	conn.compression = packetCompressionAdapterFunc(algorithm)
}
...
func (conn *ByteFrameConnection) ReadRoutine(onPacket func([]byte)) {
	pressureMetric := pressure_metric.NewPressureMetric(time.Second*3, func(e float32) {
		if e > 0.5 {
			termlog.Warnf("handle pressure: %.2f%%", e*100)
		}
	})
	for {
		pressureMetric.IdleStart()
