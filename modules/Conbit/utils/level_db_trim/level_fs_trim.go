...
type FileDesc = storage.FileDesc
type FileType = storage.FileType
type Locker = storage.Locker

const (
	TypeManifest = storage.TypeManifest
	TypeJournal  = storage.TypeJournal
	TypeTable    = storage.TypeTable
	TypeTemp     = storage.TypeTemp
)
...
func FileDescOk(fd FileDesc) bool {
	return storage.FileDescOk(storage.FileDesc(fd))
}
...
func isErrAccessDenied(err error) bool {
	if pathErr, ok := err.(*os.PathError); ok {
		return pathErr.Err == syscall.ERROR_ACCESS_DENIED
	}
	return false
}
