...
import (
	...
	"github.com/LangTuStudio/RaaBel/core/minecraft/text"
...
func newConn(netConn net.Conn, key *ecdsa.PrivateKey, log *slog.Logger, proto Protocol, flushRate time.Duration, limits bool) *Conn {
	...
	if provider, ok := netConn.(interface{ PacketHeader() (byte, bool) }); ok {
		if _, useHeader := provider.PacketHeader(); !useHeader {
...
func (conn *Conn) handleRequestNetworkSettings(pk *packet.RequestNetworkSettings) error {
	...
	conn.dec.EnableCompression(conn.maxDecompressedLen, true)
	return nil
}
...
func (conn *Conn) handleItemRegistry(pk *packet.ItemRegistry) error {
	...
	// Author: Happy2018new
	//
	// Since nemc 1.21.50, we should send this packet to notify the server we start loding the screen.
	// Otherwise, the server will block all the bot actions even though we send player auth input.
	// (e.g. click block, click air, open container, etc.)
	_ = conn.WritePacket(&packet.ServerBoundLoadingScreen{
...
func (conn *Conn) tryFinaliseClientConn() {
	if conn.waitingForSpawn.Load() && conn.gameDataReceived.Load() {
		...
		// Author: Happy2018new
		//
		// Since nemc 1.21.50, we should send this packet to notify the server we end the screen loding.
		// Otherwise, the server will block all the bot actions even though we send player auth input.
		// (e.g. click block, click air, open container, etc.)
		_ = conn.WritePacket(&packet.ServerBoundLoadingScreen{
