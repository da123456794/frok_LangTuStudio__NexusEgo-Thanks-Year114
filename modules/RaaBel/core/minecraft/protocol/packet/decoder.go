...
func NewDecoder(reader io.Reader) *Decoder {
	if pr, ok := reader.(packetReader); ok {
		return &Decoder{checkPacketLimit: true, pr: pr, readHeader: true}
	}
	return &Decoder{
		r:                reader,
...
// NewDecoderWithoutHeader returns a new Decoder that expects packet batches without the outer header byte.
func NewDecoderWithoutHeader(reader io.Reader) *Decoder {
	if pr, ok := reader.(packetReader); ok {
