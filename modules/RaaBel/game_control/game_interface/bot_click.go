...
type UseItemOnBlocks struct {
	...
	ClickedPosition mgl32.Vec3               // 指代机器人操作该方块时点击的方块内相对坐标
}
...
func (b *BotClick) clickBlock(
	...
	// Step 2: 取得当前手持物品的信息
	item, inventoryExisted := b.r.Inventories().GetItemStack(resources_control.WindowNameInventory, request.HotbarSlotID)
	if !inventoryExisted {
	...
	err := b.r.WritePacket(&packet.InventoryTransaction{
		...
		TransactionData: &protocol.UseItemTransactionData{
			...
			HeldItem:           *item,
			Position:           position,
			ClickedPosition:    request.ClickedPosition,
...
func (b *BotClick) ClickBlockWitchPosition(
	...
	_, err := b.c.SendWSCommandWithResp(fmt.Sprintf(`tp @a[name="%s"] %v %v %v`, b.r.BotName, position[0], position[1], position[2]))
	if err != nil {
		return fmt.Errorf("ClickBlockWitchPosition: %v", err)
	}

	err = b.ClickBlockWitchPositionWithoutTeleport(request, position)
	if err != nil {
		return fmt.Errorf("ClickBlockWitchPosition: %v", err)
	}
	return nil
...
此函数不会自动切换物品栏，但会等待租赁服响应更改
*/
func (b *BotClick) ClickBlockWitchPositionWithoutTeleport(
	request UseItemOnBlocks,
	position mgl32.Vec3,
) error {
	err := b.clickBlock(request, 0, position)
	if err != nil {
		return fmt.Errorf("ClickBlockWitchPositionWithoutTeleport: %v", err)
	}
	return nil
}
...
func (b *BotClick) ClickAir(hotbarSlot resources_control.SlotID, realPosition mgl32.Vec3) error {
	...
	item, inventoryExisted := b.r.Inventories().GetItemStack(resources_control.WindowNameInventory, hotbarSlot)
	if !inventoryExisted {
...
func (b *BotClick) PlaceBlock(
	...
	_, err := b.c.SendWSCommandWithResp(fmt.Sprintf(`tp @a[name="%s"] %v %v %v`, b.r.BotName, request.BotPos[0], request.BotPos[1], request.BotPos[2]))
	if err != nil {
		return fmt.Errorf("PlaceBlock: %v", err)
	}

	err = b.PlaceBlockWithoutTeleport(request, blockFace)
	if err != nil {
		return fmt.Errorf("PlaceBlock: %v", err)
	}
	return nil
...
/*
PlaceBlockWithoutTeleport 使客户端创建一个新方块。
...
此函数不会自动切换物品栏，但会等待租赁服响应更改
*/
func (b *BotClick) PlaceBlockWithoutTeleport(
	request UseItemOnBlocks,
	blockFace int32,
) error {
	err := b.clickBlock(request, blockFace, mgl32.Vec3{})
	if err != nil {
		return fmt.Errorf("PlaceBlockWithoutTeleport: %v", err)
	}
	return nil
}
