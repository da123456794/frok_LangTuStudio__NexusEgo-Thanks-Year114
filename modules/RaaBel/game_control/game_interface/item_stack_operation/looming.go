...
func (l Looming) Make(runtimeData MakingRuntime) []protocol.StackRequestAction {
	data := runtimeData.(LoomingRuntime)
...

	{
		...
		moveResult.Source = protocol.StackRequestSlotInfo{
			// [NEMC 1.20.10] 60 -> 61 (Added by Happy2018new)
			Container: protocol.FullContainerName{
	...
	result := []protocol.StackRequestAction{
		&moveBanner,
		&moveDye,
		&protocol.CraftLoomRecipeStackRequestAction{Pattern: l.PatternName, TimesCrafted: 1},
