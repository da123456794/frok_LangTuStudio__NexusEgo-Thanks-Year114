	...
	for index, requests := range allRequests {
		...
		for _, operation := range requests {
			...
			case item_stack_operation.BeaconPayment:
				result, err = handler.handleBeaconPayment(op, requestID)
			}
			if err != nil {
				return false, nil, nil, fmt.Errorf("Commit: %v", err)
			}
...
func (i *ItemStackTransaction) checkRenaming(
	...
	for index, response := range serverResponse {
		...
		cid1 := response.ContainerInfo[0].Container.ContainerID
		cid2 := response.ContainerInfo[1].Container.ContainerID
