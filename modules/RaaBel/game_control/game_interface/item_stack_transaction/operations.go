...
func dynamicContainerWindowName(dynamicContainerID resources_control.DynamicContainerID) resources_control.WindowName {
	return resources_control.NewWindowName(protocol.WindowIDDynamic, dynamicContainerID)
}

// MoveBetweenDynamicContainer 将动态容器中 source 处的物品
// 移动到同一动态容器的 destination 处，且只移动 count 个物品。
//
// dynamicContainerID 指示动态容器 ID，例如收纳袋 NBT 中的 bundle_id。
// 该操作不需要打开物品栏或普通容器。
...
func (i *ItemStackTransaction) MoveBetweenContainer(
	...
	return i.MoveItem(
		resources_control.SlotLocation{
			WindowName: resources_control.NewWindowName(windowID, 0),
			SlotID:     source,
		},
		resources_control.SlotLocation{
			WindowName: resources_control.NewWindowName(windowID, 0),
...
func (i *ItemStackTransaction) MoveToContainer(
	...
	return i.MoveItem(
		...
		resources_control.SlotLocation{
			WindowName: resources_control.NewWindowName(resources_control.WindowID(data.WindowID), 0),
			SlotID:     destination,
		},
		count,
	)
