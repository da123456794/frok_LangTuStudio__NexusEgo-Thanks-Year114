...
// allItemInstances 列出窗口名为 windowName 的库存中所有物品的网络物品堆栈信息。
// 保证 allItemInstances 在实现上是深拷贝的，这意味着使用者可以安全的修改返回值
func (v *virtualInventories) allItemInstances(windowName resources_control.WindowName) map[resources_control.SlotID]protocol.ItemInstance {
	result := make(map[resources_control.SlotID]protocol.ItemInstance)

	mapping, inventoryExisted := v.api.GetAllItemStack(windowName)
	if !inventoryExisted {
		return nil
	}
...
func (v *virtualInventories) loadItemStack(slotLocation resources_control.SlotLocation) (result protocol.ItemStack, err error) {
	...
	item, inventoryExisted := v.api.GetItemStack(slotLocation.WindowName, slotLocation.SlotID)
	if !inventoryExisted {
