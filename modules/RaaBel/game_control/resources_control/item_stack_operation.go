...
type (
	...
	// ContainerID 是容器的 ID
	ContainerID uint8
	// ContainerNameKey 是可比较的完整容器名，用于匹配物品堆栈响应。
	ContainerNameKey struct {
		ContainerID           byte
		DynamicContainerID    uint32
		HasDynamicContainerID bool
	}
)
...
type (
	// ItemStackResponseMapping 是一个由完整容器名到库存窗口名的映射。
	// 由于服务器返回的物品堆栈响应按 FullContainerName 来返回更改的物品堆栈，
	// 因此本处的资源处理器定义了下面的运行时映射，以便于操作
	ItemStackResponseMapping map[ContainerNameKey]WindowName
