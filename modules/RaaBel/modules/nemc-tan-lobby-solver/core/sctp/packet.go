...
// Create the CRC32C table used by SCTP checksums.
var crc32cTable = crc32.MakeTable(crc32.Castagnoli) // nolint:gochecknoglobals
...
func generatePacketChecksum(raw []byte) (sum uint32) {
	...
	sum = crc32.Update(sum, crc32cTable, raw[0:8])
	sum = crc32.Update(sum, crc32cTable, fourZeroes[:])
	sum = crc32.Update(sum, crc32cTable, raw[12:])
