...
var (
	...
	// 将它们进行集中性物品染色处理。应当说明的是，这些物品应当置于非快捷栏的物品栏，
	// 并且对于无需处理的物品，应当简单的置为 nil
	DyeMultiple func(console *nbt_console.Console, multipleItems [27]*nbt_parser_interface.Item) error
	// LoreMultiple 根据操作台 console 和已放入背包的多个物品 multipleItems，
	// 将它们进行一一描述处理。应当说明的是，这些物品应当置于非快捷栏的物品栏，
	// 并且对于无需处理的物品，应当简单的置为 nil
	LoreMultiple func(console *nbt_console.Console, multipleItems [27]*nbt_parser_interface.Item) error
	// EnchRenameDyeOrLoreMultiple 根据操作台 console 和已放入背包的多个物品 multipleItems，
	// 将它们进行集中性的物品附魔、物品改名、物品染色和 Lore 处理。应当说明的是，这些物品应当置于非快捷栏的物品栏，
	// 并且对于无需处理的物品，应当简单的置为 nil
	EnchRenameDyeOrLoreMultiple func(console *nbt_console.Console, multipleItems [27]*nbt_parser_interface.Item) error
	// EnchRenameDyeOrLoreSingle 根据操作台 console、目标物品 item 和其所在槽位 slot，
	// 对这个物品执行一次附魔、改名、染色和 Lore 处理
	EnchRenameDyeOrLoreSingle func(console *nbt_console.Console, item nbt_parser_interface.Item, slot resources_control.SlotID) error
)
