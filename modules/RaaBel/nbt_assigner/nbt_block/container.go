...
func (c *Container) makeNormal() error {
	...
	if len(allSubBlocks) > 0 {
		...
		allItemStack, inventoryExisted := api.Resources().Inventories().GetAllItemStack(resources_control.WindowNameInventory)
		if !inventoryExisted {
			panic("makeNormal: Should nerver happened")
		}
	...
	// Step 9.1: 找出所有需要附魔、改名、染色或 Lore 的物品
	needEnchRenameDyeOrLoreList := make([]int, 0)
	for index, value := range c.data.NBT.Items {
		if value.Item.NeedEnchRenameDyeOrLore() {
			needEnchRenameDyeOrLoreList = append(needEnchRenameDyeOrLoreList, index)
		}
	}

	// Step 9.2: 将需要特殊处理的物品移动到背包
	if len(needEnchRenameDyeOrLoreList) > 0 {
		_, err = api.Commands().SendWSCommandWithResp("clear")
		if err != nil {
			return fmt.Errorf("makeNormal: %v", err)
		}
		...
		transaction := api.ItemStackOperation().OpenTransaction()
		for _, index := range needEnchRenameDyeOrLoreList {
			item := c.data.NBT.Items[index]
			_ = transaction.MoveToInventory(
	...
	if len(needEnchRenameDyeOrLoreList) > 0 {
		...
		err = nbt_assigner_interface.EnchRenameDyeOrLoreMultiple(c.console, multipleItems)
		if err != nil {
			return fmt.Errorf("makeNormal: %v", err)
		}
	}
