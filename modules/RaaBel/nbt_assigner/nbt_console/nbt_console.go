...
func NewConsole(api *game_interface.GameInterface, dimensionID uint8, center protocol.BlockPos) (result *Console, err error) {
	return NewConsoleWithResponseMode(api, dimensionID, center, true)
}

func NewConsoleWithResponseMode(api *game_interface.GameInterface, dimensionID uint8, center protocol.BlockPos, waitCommandResponse bool) (result *Console, err error) {
	c := &Console{api: api}

	err = c.initConsole(dimensionID, center, waitCommandResponse)
	if err != nil {
...
func (c *Console) initConsole(dimensionID uint8, center protocol.BlockPos, waitCommandResponse bool) error {
	api := c.api
	var err error
	...
	*c = Console{
	...
	if api.Maintainer() != nil {
		err := api.Maintainer().TouchMaintainer(api)
		if err != nil {
	...
	dimensionName := utils.DimensionNameByID(dimensionID)
	err = api.Commands().SendSettingsCommand(
		fmt.Sprintf("execute in %s run tp %d %d %d", dimensionName, c.center[0], c.center[1], c.center[2]),
		true,
	)
	if err != nil {
		return fmt.Errorf("initConsole: %v", err)
	...
	// Init console blocks
	{
		...
		if waitCommandResponse {
			_, err = api.Commands().SendWSCommandWithResp(cleanCommand)
			if err != nil {
				return fmt.Errorf("initConsole: %v", err)
			}
		} else {
			err = api.Commands().SendSettingsCommand(cleanCommand, false)
			if err != nil {
				return fmt.Errorf("initConsole: %v", err)
			}
		...
		if waitCommandResponse {
			_, err = api.Commands().SendWSCommandWithResp(floorCommand)
			if err != nil {
				return fmt.Errorf("initConsole: %v", err)
			}
		} else {
			err = api.Commands().SendSettingsCommand(floorCommand, false)
			if err != nil {
				return fmt.Errorf("initConsole: %v", err)
			}
			time.Sleep(200 * time.Millisecond)
