...
import (
	...
	nbt_parser_general "github.com/LangTuStudio/RaaBel/nbt_parser/general"
...
func (s *Shield) Make() (resultSlot map[uint64]resources_control.SlotID, err error) {
	...
	for index := range bannerSlots {
		transaction.
			...
			Crafting(3250, shieldSlots[index], 1, shieldExpectedItem[index])
	}
	...
	for index, shieldSlot := range shieldSlots {
		...
		shieldWeGet, inventoryExisted := api.Resources().Inventories().GetItemStack(resources_control.WindowNameInventory, shieldSlot)
		if !inventoryExisted {
