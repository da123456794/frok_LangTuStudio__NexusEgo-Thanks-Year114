...
func (d *DefaultItem) Format(prefix string) string {
	...
	if len(d.Enhance.Lore) > 0 {
		result += prefix + "\t" + fmt.Sprintf("物品描述: %s\n", strings.Join(d.Enhance.Lore, " "))
	}
	if d.Enhance.CustomColor != nil {
		result += prefix + "\t" + fmt.Sprintf("染色 RGB 值: %v\n", d.Enhance.CustomColor)
	}
...
func (d DefaultItem) NeedEnchRenameDyeOrLore() bool {
...
func (d *DefaultItem) TypeStableBytes() []byte {
	...
	// Enhance (Display Name, Ench List, Lore, CustomColor)
	w.String(&d.Enhance.DisplayName)
	protocol.SliceUint16Length(w, &d.Enhance.EnchList)
	protocol.FuncSliceUint16Length(w, &d.Enhance.Lore, w.String)
	haveCustomColor := d.Enhance.CustomColor != nil
	w.Bool(&haveCustomColor)
	if haveCustomColor {
		dyeIDs, _ := utils.SearchCauldronDyeIDsByColor(*d.Enhance.CustomColor)
		w.ByteSlice(&dyeIDs)
	}
