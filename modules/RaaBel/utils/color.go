...
// CalculateColorDistance 计算两个 RGB 颜色 colorA 和 colorB 的欧式距离
...
// SearchForBestColor 从 mapping 中选出距离 color 最近的 RGB 颜色
...
// DecodeVarRGBA 从 x 解码一个 RGBA 颜色
...
// MixTwoColors 两种颜色按 MC 炼药锅规则混合：(a + b) / 2
func MixTwoColors(a, b [3]uint8) [3]uint8 {
	r := (uint16(a[0]) + uint16(b[0])) / 2
	g := (uint16(a[1]) + uint16(b[1])) / 2
	bVal := (uint16(a[2]) + uint16(b[2])) / 2
	return [3]uint8{uint8(r), uint8(g), uint8(bVal)}
}

// MixColorSequence 按顺序混合一系列颜色，每一步都是 (当前色 + 新色)/2
func MixColorSequence(colors ...[3]uint8) [3]uint8 {
	if len(colors) == 0 {
		return [3]uint8{}
	}
	current := colors[0]
	for i := 1; i < len(colors); i++ {
		current = MixTwoColors(current, colors[i])
	}
	return current
}
