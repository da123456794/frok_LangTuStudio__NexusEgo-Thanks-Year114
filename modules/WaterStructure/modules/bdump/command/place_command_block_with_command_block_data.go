...
func (_ *PlaceCommandBlockWithCommandBlockData) Name() string {
	return NamePlaceCommandBlockWithCommandBlockData
}
...
func (cmd *PlaceCommandBlockWithCommandBlockData) Marshal(writer io.Writer) error {
	...
	binary.BigEndian.PutUint32(uint32Buf, cmd.CommandBlockData.Mode)
	...
	binary.BigEndian.PutUint32(uint32Buf, uint32(cmd.CommandBlockData.TickDelay))
	_, err = writer.Write(uint32Buf)
	if err != nil {
		return err
	}
	binary.BigEndian.PutUint32(uint32Buf, 0) // cleanup the buffer
...
func (cmd *PlaceCommandBlockWithCommandBlockData) Unmarshal(reader io.Reader) error {
	...
	cmd.BlockData = binary.BigEndian.Uint16(uint16Buf)
