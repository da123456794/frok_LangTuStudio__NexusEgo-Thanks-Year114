...
func VerifyStreamBDX(stream io.Reader) (bool, bool, string, error) {
	lastBlock := make([]byte, 2048)
	curBlock := make([]byte, 2048)
	afterFirstRun := false
	hash := sha256.New()
	for {
		if afterFirstRun {
			copy(lastBlock, curBlock)
		}
		n, _ := io.ReadAtLeast(stream, curBlock, 2048)
		if n != 2048 {
			var bufferedBlock []byte
			if n == 0 {
				bufferedBlock = lastBlock
			} else {
				if !afterFirstRun {
					bufferedBlock = curBlock[:n]
				} else {
					bufferedBlock = append(lastBlock, curBlock[:n]...)
				}
			}
			bbl := len(bufferedBlock)
			if bufferedBlock[bbl-1] != 90 {
				// Not signed
				return false, false, "", nil
			}
			signLen := int(bufferedBlock[bbl-2])
			var sign []byte
			var bodyLeft []byte
			if signLen == int(255) {
				signLenBuf := bufferedBlock[bbl-4 : bbl-2]
				signLen = int(binary.BigEndian.Uint16(signLenBuf))
				if signLen >= bbl-4 {
					return false, false, "", fmt.Errorf("Too long signature")
				}
				sign = bufferedBlock[bbl-signLen-4 : bbl-4]
				bodyLeft = bufferedBlock[:bbl-signLen-5]
			} else {
				sign = bufferedBlock[bbl-signLen-2 : bbl-2]
				bodyLeft = bufferedBlock[:bbl-signLen-3]
			}
			hash.Write(bodyLeft)
			hashVal := hash.Sum(nil)
			cor, usn, err := VerifyBDX(hashVal, sign)
			return true, cor, usn, err
		}
		if afterFirstRun {
			hash.Write(lastBlock)
		} else {
			afterFirstRun = true
		}
	}
