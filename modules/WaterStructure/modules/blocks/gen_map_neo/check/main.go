...
func main() {
	...
	fmt.Println(blocks.RuntimeIDToBlockNameAndStateStr(rtid))
	rtid, ok = blocks.BlockStrToRuntimeID(`wool 4`)
	if !ok {
		panic("not ok")
	}
	fmt.Println(blocks.RuntimeIDToBlockNameAndStateStr(rtid))
	rtid, ok = blocks.BlockStrToRuntimeID(`shulker_box 3`)
	if !ok {
		panic("not ok")
	}
	fmt.Println(blocks.RuntimeIDToBlockNameAndStateStr(rtid))

	rtid, ok = blocks.BlockStrToRuntimeID("carved_pumpkin[facing=east]")
	if !ok {
		panic("not ok")
	}
	fmt.Println(blocks.RuntimeIDToBlockNameAndStateStr(rtid))
