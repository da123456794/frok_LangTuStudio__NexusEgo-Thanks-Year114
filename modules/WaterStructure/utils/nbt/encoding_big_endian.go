...
func (e bigEndian) WriteString(w *offsetWriter, x string) error {
	...
	if length > maxStringSize {
		return InvalidStringError{Off: w.off, N: uint(length), Err: errStringTooLong}
	}
	if err := e.WriteInt16(w, int16(uint16(length))); err != nil {
		return FailedWriteError{Op: "WriteString", Off: w.off}
	}
	if length == 0 {
		return nil
	}
	if _, err := w.Write(stringBytes(x)); err != nil {
		return FailedWriteError{Op: "WriteString", Off: w.off}
	}
	return nil
}
