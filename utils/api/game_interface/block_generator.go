...
func (g *GameInterface) GenerateNewAnvil(pos [3]int32, blockStates string) (
	...
	// 备份相关的方块
	err = g.SendAICommand(fmt.Sprintf("setblock %d %d %d %s", pos[0], pos[1]-1, pos[2], AnvilBase), true)
	if err != nil {
		return uuid.UUID{}, [3]int32{}, fmt.Errorf("GenerateNewAnvil: %v", err)
	}
	err = g.SendAICommand(fmt.Sprintf("setblock %d %d %d anvil %s", pos[0], pos[1], pos[2], blockStates), true)
	if err != nil {
		return uuid.UUID{}, [3]int32{}, fmt.Errorf("GenerateNewAnvil: %v", err)
	}
...
func (g *GameInterface) PlaceBlockWithFacing(
	...
	if pos != originPos {
		...
		defer g.RevertStructure(backupShulkerBoxUniqueId, BlockPos{int(pos[0]), int(pos[1]), int(pos[2])})
	}
	// 可能目标方块并非生成在原本给定的坐标处，
	// 此时需要进行特殊处理
	...
	err = g.SendAICommand(teleportCommand, true)
	if err != nil {
		return fmt.Errorf("PlaceBlockWithFacing: %v", err)
	}
	uniqueId, err := g.BackupStructure(
		MCStructure{
			BeginX: int(backupBlockPos[0]),
			BeginY: int(backupBlockPos[1]),
			BeginZ: int(backupBlockPos[2]),
			SizeX:  1,
			SizeY:  1,
			SizeZ:  1,
		},
	...
	err = g.RevertStructure(uniqueId, BlockPos{int(backupBlockPos[0]), int(backupBlockPos[1]), int(backupBlockPos[2])})
	if err != nil {
		return fmt.Errorf("PlaceBlockWithFacing: %v", err)
	}
