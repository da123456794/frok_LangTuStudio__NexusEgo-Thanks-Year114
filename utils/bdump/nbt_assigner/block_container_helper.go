...
func (c *Container) OpenContainer() (bool, error) {
	...
	if strings.Contains(c.BlockEntity.Block.Name, "shulker_box") || strings.Contains(c.BlockEntity.Block.Name, "chest") {
		...
		defer api.RevertStructure(uniqueId, GameInterface.BlockPos{
			X: int(backupBlockPos[0]),
			Y: int(backupBlockPos[1]),
			Z: int(backupBlockPos[2]),
		})
...
func (c *Container) GetSubBlock(
	...
	defer api.RevertStructure(uniqueId, GameInterface.BlockPos{
		X: int(c.BlockEntity.AdditionalData.Position[0]),
		Y: int(c.BlockEntity.AdditionalData.Position[1]),
		Z: int(c.BlockEntity.AdditionalData.Position[2]),
	})
	// 备份容器
