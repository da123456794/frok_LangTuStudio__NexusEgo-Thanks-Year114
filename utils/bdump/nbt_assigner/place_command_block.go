...
func (c *CommandBlock) WriteData() error {
	...
	// 放置命令方块
	// 如果不要求写入命令方块数据
	err := c.BlockEntity.Interface.SendAICommand(fmt.Sprintf("tp %d %d %d", c.BlockEntity.AdditionalData.Position[0], c.BlockEntity.AdditionalData.Position[1], c.BlockEntity.AdditionalData.Position[2]), true)
	if err != nil {
		return fmt.Errorf("WriteData: %v", err)
	}
	// 传送机器人到命令方块处
...
func (c *CommandBlock) PlaceCommandBlockLegacy(
	...
	request := commands_generator.SetBlockRequest(block, cfg, &client.Client{})
	if c.BlockEntity.AdditionalData.FastMode {
		err := api.SendAICommand(request, true)
		if err != nil {
