...
// PeekBytes peeks ahead n bytes without consuming them. If fewer than n bytes are buffered, the bytes
// available are returned.
func (r *Reader) PeekBytes(n int) []byte {
	if br, ok := r.r.(*bytes.Buffer); ok {
		b := br.Bytes()
		if len(b) < n {
			n = len(b)
		}
...
// TakeRemainingBytes reads and returns all remaining bytes from the reader.
...
func (r *Reader) NeteasePixels(x *[]color.RGBA) {
	...
	case 1:
		...
		var length uint32
		r.Varuint32(&length)
		colours = make([]color.RGBA, length)
...
func (r *Reader) NBTItem(m *Item) {
	...
	if len(enchList) > 0 {
		...
		newVal := make([]any, len(nbtEnchList))
...
func (r *Reader) EnchantList(m *[]Enchant) {
	...
	SliceVarint16Length[Enchant, *[]Enchant, *Enchant](r, m)
}
...
// MsgPack reads a NetEase/Python style msgpack payload and normalises nested maps.
