...
func (x Skin) validate() error {
	if len(x.SkinData) != 0 && x.SkinImageHeight*x.SkinImageWidth*4 != uint32(len(x.SkinData)) {
	...
	if len(x.CapeData) != 0 && x.CapeImageHeight*x.CapeImageWidth*4 != uint32(len(x.CapeData)) {
	...
	for i, animation := range x.Animations {
		if len(animation.ImageData) != 0 && animation.ImageHeight*animation.ImageWidth*4 != uint32(len(animation.ImageData)) {
