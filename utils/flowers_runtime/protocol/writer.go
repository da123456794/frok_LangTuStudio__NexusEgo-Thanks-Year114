...
func (w *Writer) NBTWithLength(x *map[string]any) {
	...
	if err := nbt.NewEncoderWithEncoding(buf, nbt.NetworkLittleEndian).Encode(*x); err != nil {
		panic(err)
	}
	bufferLength := uint32(buf.Len())
	w.Varuint32(&bufferLength)
	bytesVal := buf.Bytes()
	w.Bytes(&bytesVal)
}
...
func (w *Writer) NBTItem(x *Item) {
	...
	if x.Tag != nil {
		if ench, hasEnch := x.Tag["ench"].([]any); hasEnch {
			if err := mapstructure.Decode(ench, &enchList); err != nil {
				panic(err)
			}
		}
	}
...
func (w *Writer) ItemList(m *[]ItemWithSlot) {
	newBytes := bytes.NewBuffer(nil)
	writer := NewWriter(newBytes, 0)
	for _, item := range *m {
		current := item
		current.Marshal(writer)
	}
	bytesVal := newBytes.Bytes()
	w.ByteSlice(&bytesVal)
}

func (w *Writer) EnchantList(m *[]Enchant) {
	bufferLength := uint16(len(*m))
	w.Varuint16(&bufferLength)
	if bufferLength == 0 {
		return
	}
	SliceVarint16Length[Enchant, *[]Enchant, *Enchant](w, m)
}

func (w *Writer) MapPixelsDataType(x *MapPixelsData) {
	var pixelsType uint8
	if !lookupMapPixelsType(*x, &pixelsType) {
