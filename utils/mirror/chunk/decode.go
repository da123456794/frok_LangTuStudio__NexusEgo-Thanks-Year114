...
import (
	...
	"github.com/LangTuStudio/Conbit/minecraft/protocol/block_actors"
...
func NEMCSubChunkDecode(data []byte) (int8, *SubChunk, []map[string]interface{}, error) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			if strings.Contains(errMsg, "Target block not found in pool") {
				return
			} else {
				panic(r)
			}
	...
	if err != nil {
		return subChunkIndex, nil, nil, err
	}
	...
	return subChunkIndex, subChunk, nbts, nil
}
...
func NEMCTagNBTDecode(ID string, tag string) (result map[string]any, err error) {
	defer func() {
		if r := recover(); r != nil {
			result = nil
			err = fmt.Errorf("NEMCTagNBTDecode: %v", r)
		}
	}()
