...
import (
	...
	"nexus/utils/mirror"
	"nexus/utils/mirror/chunk"
	"nexus/utils/mirror/define"
	"sync"
	"time"
...
type Assembler struct {
	...
	retryMu          sync.Mutex
	retryCount       map[define.ChunkPos]int
	retryAt          map[define.ChunkPos]time.Time
	maxRetry         int
...
func NewAssembler(sendPeriod time.Duration, validCacheTime time.Duration) *Assembler {
	...
	a := &Assembler{
		...
		centerChunk:      nil,
		radius:           10,
		retryCount:       make(map[define.ChunkPos]int),
		retryAt:          make(map[define.ChunkPos]time.Time),
		maxRetry:         4,
...
func (o *Assembler) buildFullSubChunkRequest(cp define.ChunkPos, dimension int32) *packet.SubChunkRequest {
	offsets := make([]protocol.SubChunkOffset, 24)
	for i := -4; i <= 19; i++ {
		offsets[i+4] = [3]int8{0, int8(i), 0}
	}
	return &packet.SubChunkRequest{
		Dimension: dimension,
		Position:  protocol.SubChunkPos{cp.X(), 0, cp.Z()},
		Offsets:   offsets,
	}
}

func (o *Assembler) scheduleRetry(cp define.ChunkPos, dimension int32) bool {
	o.retryMu.Lock()
	if o.maxRetry <= 0 {
		o.retryMu.Unlock()
		return false
	...
	now := time.Now()
	if last, ok := o.retryAt[cp]; ok && now.Sub(last) < o.retryInterval {
		o.retryMu.Unlock()
		return true
	}
	count := o.retryCount[cp]
	if count >= o.maxRetry {
		delete(o.retryCount, cp)
		delete(o.retryAt, cp)
		o.retryMu.Unlock()
		return false
	}
	o.retryCount[cp] = count + 1
	o.retryAt[cp] = now
	o.retryMu.Unlock()

	o.ScheduleRequest([]*packet.SubChunkRequest{o.buildFullSubChunkRequest(cp, dimension)})
	return true
}
...
func (o *Assembler) OnNewSubChunk(pk *packet.SubChunk) *mirror.ChunkData {
	...
	*/
	...
	if chunkData, hasK := o.pendingTasks[cp]; !hasK {
		...
		o.taskMu.RUnlock()
		assignAllAir := func(index int8) {
			ai := int(index) + 4
			if ai < 0 || ai >= len(chunkData.Chunk.Sub()) {
				fmt.Printf("SUBCHUNK INDEX OUT OF RANGE: %d for %v\n", index, cp)
				return
			}
			allAirSubChunk := chunk.NewSubChunk(o.airRID)
			allAirSubChunk.Validate()
			chunkData.Chunk.AssignSub(ai, allAirSubChunk)
		}
		hasMissing := false
		for _, entry := range pk.SubChunkEntries {
			expectedIndex := int8(pk.Position[1]) + entry.Offset[1]
			switch entry.Result {
			case protocol.SubChunkResultSuccess:
				_, subChunk, nbts, err := chunk.NEMCSubChunkDecode(entry.RawPayload)
				if err != nil || subChunk == nil {
					fmt.Printf("SUBCHUNK DECODE ERR: %v (%v)\n", err, cp)
					hasMissing = true
					continue
				}
				ai := int(expectedIndex) + 4
				if ai < 0 || ai >= len(chunkData.Chunk.Sub()) {
					fmt.Printf("SUBCHUNK INDEX OUT OF RANGE: %d for %v\n", expectedIndex, cp)
					hasMissing = true
					continue
				}
				chunkData.Chunk.AssignSub(ai, subChunk)
				...
				continue
			case protocol.SubChunkResultSuccessAllAir, protocol.SubChunkResultIndexOutOfBounds:
				assignAllAir(expectedIndex)
				continue
		...
		for _, subChunk := range subs {
			if subChunk == nil || subChunk.Invalid() {
				emptySubChunkCounter++
				//return nil
			}
		}
		if hasMissing || emptySubChunkCounter != 0 {
			if o.scheduleRetry(cp, pk.Dimension) {
				return nil
			}
